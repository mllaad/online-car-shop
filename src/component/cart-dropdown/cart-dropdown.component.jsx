...
const CartDropDown = ({ logoEl }) => {
  ...
  const handleClickOutside = (event) => {
    const domEl =
      logoEl.current.contains(event.target) ||
  ...
  return (
    <div
      ref={dropdownEl}
      className={`cart-dropdown-container ${toggleCart && "togglecart"}`}
    >
      <div className="cart-items">
        {cartItems.map((item, key) => {
          const { CarModel, Price, ModleYear, img, quantity } = item;
          return (
            <div className="empty-message" key={key}>
              <img className="dropdown-img" src={img} alt={CarModel} />
              <div className="dropdown-detail">
                <p className="dropdown__title">{CarModel}</p>
                <p className="dropdown__year">{ModleYear}</p>
